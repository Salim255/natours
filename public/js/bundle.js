...
var displayMap = function displayMap(location) {
  mapboxgl.accessToken = 'pk.eyJ1Ijoic2FsaW0yMyIsImEiOiJja3VwOW9ybWExYXU0Mm50aGM4M3ZyY2hwIn0.J7ideEPvsBHJZMkVePAv4A';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/salim23/ckupi8t0z0i7x18qvj4y6xm88',
    scrollZoom: false // center: [3.057256, 50.62925],
    // zoom: 10,
    // interactive: false
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, " </p>")).addTo(map); //Extebd map bounds to include the current location
...
var showAlert = function showAlert(type, msg) {
  ...
  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); //in side of the boy rightup in the beginning
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 7:
              ///In order we nead that await then ...to clear the the password field in our front end
