...
const userSchema = new mongoose.Schema({
  ...
  active: {
    type: Boolean,
    default: true,
    select: false, //to don't be shown on the user information
  },
});
...
// Its basically a method gonna be available on all documents of certain collection
...
userSchema.methods.changedPasswordAfter = function (JWTTimestamp) {
  if (this.passwordChangedAt) {
    ...
    return JWTTimestamp < changedTimestamp; // If the JWT < changed that is means there are a change of the password after the token been isssued
  }
...
userSchema.methods.createPasswordResetToken = function () {
  // w'll use this function in authentication controller
  ...
  // Now we should encrypt this resetToken using built-in crypto module.
